...
# 代码实现了dropout的操作，用于在训练神经网络时随机丢弃一部分神经元，从而防止过拟合
...
# 定义了一个 PyTorch 模块 _LayerNorm，它是实现层归一化的基类
...
# GroupNorm 将通道按照分组方式进行划分，每组通道数相等，然后在组内进行标准化
def GlobLN(nOut):
    return nn.GroupNorm(1, nOut, eps=1e-8)

# 把Conv1d卷积，GroupNorm归一化，PReLU激活函数，规范到一个类中
...
# 把Conv1d卷积，GroupNorm归一化,规范到一个类中
...
# 把GroupNorm归一化，PReLU激活函数，规范到一个类中
